...
@description('Enable creation of a dedicated development deployment slot. Requires Standard or Premium plan.')
param enableDevelopmentSlot bool = false

@description('Deployment slot name created when development slot is enabled.')
...
var appServicePlanTier = contains(['B1', 'B2', 'B3'], appServicePlanSkuName) ? 'Basic' : contains(['S1', 'S2', 'S3'], appServicePlanSkuName) ? 'Standard' : 'PremiumV3'
var supportsDeploymentSlots = appServicePlanTier != 'Basic'
...
resource webAppDevelopmentSlot 'Microsoft.Web/sites/slots@2023-12-01' = if (enableDevelopmentSlot && supportsDeploymentSlots) {
  name: developmentSlotName
  parent: webApp
  location: location
  kind: 'app,linux,container'
  identity: {
...
output developmentSlotEnabled bool = enableDevelopmentSlot && supportsDeploymentSlots
output developmentSlotName string = enableDevelopmentSlot && supportsDeploymentSlots ? webAppDevelopmentSlot!.name : ''
output developmentSlotDefaultHostName string = enableDevelopmentSlot && supportsDeploymentSlots ? webAppDevelopmentSlot!.properties.defaultHostName : ''
output developmentSlotPrincipalId string = enableDevelopmentSlot && supportsDeploymentSlots ? (webAppDevelopmentSlot!.identity.principalId ?? '') : ''
